...
// ensureStreamTrackedLocked is ensureStreamTracked's body for callers that
// already hold dls.mu. DownloadWithPriority uses this instead of
// ensureStreamTracked so registration happens only after the
// stopping/closed checks under the lock have passed — it must not register a
// stream (or create any other side effect) while a StopAll teardown is still
// in flight.
