...
// the read head to keep the DFS cache under its disk limit. It keeps the
// in-memory totalSize counter in sync so IsOverBudget() stays accurate
// between evict() cleanup cycles (which re-derive it from disk); the
